...
class Settings(BaseSettings):
    ...
    openai_api_base: Optional[str] = Field(
        None,
        description="OpenAI API 基础 URL，用于使用代理或自定义端点"
    )
    llm_model: str = Field(
        default="gpt-4o-mini",
        description="用于文本生成和推理的语言模型"
    )
    embedding_model: str = Field(
        default="text-embedding-3-large",
        description="用于生成文本嵌入向量的模型"
    )
    ...
    milvus_uri: str = Field(
        ...
        description="Milvus 向量数据库连接 URI"
    )
    milvus_token: Optional[str] = Field(
        None,
        description="Milvus 认证令牌（如果使用 Milvus Cloud）"
    )
    milvus_api_key: Optional[str] = Field(
        None,
        description="Milvus API 密钥（可选认证方式）"
    )
    
    # ========== RAG-Anything 配置 ==========
    rag_working_dir: str = Field(
        default="./data/rag_storage",
        description="RAG-Anything 存储工作目录的路径"
    )
    rag_workspace: str = Field(
        default="medical",
        description="RAG-Anything 工作空间名称，用于隔离不同项目"
    )
    ...
    def validate_milvus_uri(cls, v: str) -> str:
        ...
        if not v.startswith(("http://", "https://")):
            raise ValueError(
                f"Milvus URI 必须以 http:// 或 https:// 开头，当前值：{v}"
            )
        return v
